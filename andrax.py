# GW HANYA MERANGKAI CODE BERCODE UNTUK MEMBUAT SUATU PROGRAM/SCRIPT !
# AND GW AFYA AH TAI BET SI AJG
# NAME FILE  : andrax.py ( andrax MBF )                                    
# Source By   : MOCH IKIW ZUAN XD ( Python Code )                  
# Last Update : Sabtu 22 Januari 2023                                                                       
...
class login:
    ...
    def logoo(self):
        self.clear()
        prints(Panel(f"""\n{RC}{RC}		____ ____ ____ ____ ___  ____ ____ _  _ 
        	|___ |__| |    |___ |__] |  | |  | |_/  
...
class MulaiTools:
    ...
    def logoos(self):
        self.clear()
        prints(Panel(f"""\n{RC}		____ ____ ____ ____ ___  ____ ____ _  _ 
        	|___ |__| |    |___ |__] |  | |  | |_/  
        ...
        with XyaaCode(max_workers=30) as xc:
             for UserAkun in ID:
                  try:
                         ...
                         pwidd = nama.split(' ')[0]
                         pwx = ['tasikmalaya','persib1933','bandung','jakarta','indonesia','bismillah','anjing','sayang','bagong']
                         if len(nama) <=5:
